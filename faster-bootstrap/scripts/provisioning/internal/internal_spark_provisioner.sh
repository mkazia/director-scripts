...
# 1. Set a SPARK2_PARCEL_URL environment variable.
# 2. Supply an argument that is a SPARK2_PARCEL_URL.

# This script will have to be re-run for each parcel you want to cache on the
# image that you are building.

if [ -z "${SPARK2_PARCEL_URL+set}" ]
then
  if [ "$#" -ne 1 ]
  then
    echo "Usage: $0 <parcel-url>"
    echo ""
    echo "Alternatively, set the environment variable SPARK2_PARCEL_URL prior to"
...
PARCEL_NAME="${SPARK2_PARCEL_URL##*/}"
PARCEL_DIR="${PARCEL_NAME%-*}"
CSD_NAME="${SPARK2_CSD_URL##*/}"

echo "Downloading CSD from $SPARK2_CSD_URL"
sudo -E /usr/bin/curl -s -S "${SPARK2_CSD_URL}" -o "/opt/cloudera/csd/$CSD_NAME"

echo "Downloading parcel from $SPARK2_PARCEL_URL"
sudo -E /usr/bin/curl -s -S "${SPARK2_PARCEL_URL}" -o "/opt/cloudera/parcel-repo/$PARCEL_NAME"
sudo -E /usr/bin/curl -s -S "${SPARK2_PARCEL_URL}.sha1" -o "/opt/cloudera/parcel-repo/$PARCEL_NAME.sha1"
...
then
  ...
  sudo ln -s $PARCEL_DIR /opt/cloudera/parcels/SPARK2
  sudo touch /opt/cloudera/parcels/SPARK2/.dont_delete
  echo "Done"
fi
