...
# 1. Set a KAFKA_PARCEL_URL environment variable.
# 2. Supply an argument that is a KAFKA_PARCEL_URL.

# This script will have to be re-run for each parcel you want to cache on the
# image that you are building.

if [ -z "${KAFKA_PARCEL_URL+set}" ]
then
  if [ "$#" -ne 1 ]
  then
    echo "Usage: $0 <parcel-url>"
    echo ""
    echo "Alternatively, set the environment variable KAFKA_PARCEL_URL prior to"
...
PARCEL_NAME="${KAFKA_PARCEL_URL##*/}"
PARCEL_DIR="${PARCEL_NAME%-*}"

echo "Downloading parcel from $KAFKA_PARCEL_URL"
sudo -E /usr/bin/curl -s -S "${KAFKA_PARCEL_URL}" -o "/opt/cloudera/parcel-repo/$PARCEL_NAME"
sudo -E /usr/bin/curl -s -S "${KAFKA_PARCEL_URL}.sha1" -o "/opt/cloudera/parcel-repo/$PARCEL_NAME.sha1"
...
then
  ...
  sudo ln -s $PARCEL_DIR /opt/cloudera/parcels/KAFKA
  sudo touch /opt/cloudera/parcels/KAFKA/.dont_delete
  echo "Done"
fi
