...
# 1. Set a KEYTRUSTEE_KMS_PARCEL_URL environment variable.
# 2. Supply an argument that is a KEYTRUSTEE_KMS_PARCEL_URL.

# This script will have to be re-run for each parcel you want to cache on the
# image that you are building.

if [ -z "${KEYTRUSTEE_KMS_PARCEL_URL// }" ]
then
  if [ "$#" -ne 1 ]
  then
    echo "Usage: $0 <parcel-url>"
    echo ""
    echo "Alternatively, set the environment variable KEYTRUSTEE_KMS_PARCEL_URL prior to"
    echo "running this script."
    echo "Silently exiting without setting up KeyTrustee KMS  parcel"
    exit 0
  else
    KEYTRUSTEE_KMS_PARCEL_URL=$1
  fi
fi
...
PARCEL_NAME="${KEYTRUSTEE_KMS_PARCEL_URL##*/}"
PARCEL_DIR="${PARCEL_NAME%-*}"

echo "Downloading parcel from $KEYTRUSTEE_KMS_PARCEL_URL"
sudo -E /usr/bin/curl -s -S "${KEYTRUSTEE_KMS_PARCEL_URL}" -o "/opt/cloudera/parcel-repo/$PARCEL_NAME"
sudo -E /usr/bin/curl -s -S "${KEYTRUSTEE_KMS_PARCEL_URL}.sha" -o "/opt/cloudera/parcel-repo/$PARCEL_NAME.sha"
sudo cp "/opt/cloudera/parcel-repo/$PARCEL_NAME.sha" "/opt/cloudera/parcel-repo/$PARCEL_NAME.sha1"
...
then
  ...
  sudo ln -s $PARCEL_DIR /opt/cloudera/parcels/KEYTRUSTEE
  sudo touch /opt/cloudera/parcels/KEYTRUSTEE/.dont_delete
  echo "Done"
fi
