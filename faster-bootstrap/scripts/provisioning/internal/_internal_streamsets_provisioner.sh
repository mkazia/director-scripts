...
# 1. Set a STREAMSETS_PARCEL_URL environment variable.
# 2. Supply an argument that is a STREAMSETS_PARCEL_URL.

# This script will have to be re-run for each parcel you want to cache on the
# image that you are building.

if [ -z "${STREAMSETS_PARCEL_URL+set}" ]
then
  if [ "$#" -ne 1 ]
  then
    echo "Usage: $0 <parcel-url>"
    echo ""
    echo "Alternatively, set the environment variable STREAMSETS_PARCEL_URL prior to"
    echo "running this script."
    echo "Silently exiting without setting up Streamsets parcel"
    exit 0
  else
    STREAMSETS_PARCEL_URL=$1
  fi
fi
...
PARCEL_NAME="${STREAMSETS_PARCEL_URL##*/}"
PARCEL_DIR="${PARCEL_NAME%-*}"
CSD_NAME="${STREAMSETS_CSD_URL##*/}"

echo "Downloading CSD from $STREAMSETS_CSD_URL"
sudo -E /usr/bin/curl -s -S "${STREAMSETS_CSD_URL}" -o "/opt/cloudera/csd/$CSD_NAME"

echo "Downloading parcel from $STREAMSETS_PARCEL_URL"
sudo -E /usr/bin/curl -s -S "${STREAMSETS_PARCEL_URL}" -o "/opt/cloudera/parcel-repo/$PARCEL_NAME"
sudo -E /usr/bin/curl -s -S "${STREAMSETS_PARCEL_URL}.sha" -o "/opt/cloudera/parcel-repo/$PARCEL_NAME.sha"
...
then
  ...
  sudo ln -s $PARCEL_DIR /opt/cloudera/parcels/STREAMSETS_DATACOLLECTOR
  sudo touch /opt/cloudera/parcels/STREAMSETS_DATACOLLECTOR/.dont_delete
  echo "Done"
fi
